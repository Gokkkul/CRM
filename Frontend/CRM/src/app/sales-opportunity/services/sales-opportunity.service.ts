...
export class SalesOpportunityService {
  ...
  private salesOpportunitySubject = new BehaviorSubject<ISalesOpportunity[]>(
    this.salesOpportunities
  );
  salesOpportunity$ = this.salesOpportunitySubject.asObservable();

  private salesOpportunityByStageSubject = new BehaviorSubject<any>([]);
  salesOpportunityByStage$ = this.salesOpportunityByStageSubject.asObservable();
  ...
  getSalesOpportunities() {
    this.http
      .get(this.apiUrl + '/get-sales-opportunities')
      .subscribe((data: any) => {
  ...
  updateSalesOpportunity(data: ISalesOpportunity, index: number) {
    ...
    return this.http.put(
      this.apiUrl + `/update-sales-opportunity/${id}`,
      updatedObj
    );
  }
  ...
  deleteSalesOpportunity(index: number) {
    ...
    this.salesOpportunities = this.salesOpportunities.filter(
      (item) => item.id !== id
    );
    this.salesOpportunitySubject.next(this.salesOpportunities);

    return this.http
      .delete(this.apiUrl + `/delete-sales-opportunity/${id}`)
      .subscribe(() => {
        console.log('Deleted Sales Opportunity');
        this.swal.showToast(
          'Sales Opportunity Deleted Successfully.',
          'success'
        );
      });
  }
  ...
  addLeadToSalesOpportunity() {
    this.http
      .post(this.apiUrl + `/add-lead-to-sales-opportunity`, '')
      .subscribe({
        next: () => {
          this.getSalesOpportunities();
        ...
        error: () => {
          this.swal.showToast(
            'Failed to Add Lead to Sales Opportunity',
            'error'
          );
        },
      });
  ...
  getSalesOpportunitiesByStage() {
    this.http
      .get(this.apiUrl + `/get-sales-opportunities-by-stage`)
      .subscribe((data) => {
        this.salesOpportunityByStageSubject.next(data);
      });
  }
}
